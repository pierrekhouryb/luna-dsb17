...
def disp_image_3axis(image, zf, xf, yf, title='', with_stats = False, save=False):
    (Z, X, Y) = image.shape
    (z, x, y) = map(lambda dim, frac: round(dim*frac), (Z,X,Y), (zf,xf,yf))
    fig, ax = plt.subplots(1,3,num=None, figsize=(16,12.8), dpi=80, facecolor='w', edgecolor='k')
...
for i, p in enumerate(list_patients):
    ...
    #Preprocess the scan
    image = imageprocessing.preprocess_scan(image, scan, do_resample=False, do_normalize=True, do_zerocenter=True)
    #Check and display
